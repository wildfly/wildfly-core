...
public class ServerInventoryImpl implements ServerInventory {
    ...
    public void serverProcessStopped(final String serverProcessName) {
        ...
        final ManagedServer server = servers.get(serverName);
        if(server == null) {
            //If we are reloading the Host Controller, we can receive this notification
            //generated by the previous ServerInventory instance which stopped the server
            //We can receive this notification when the servers have not been reconnected to the
            //new ServerInventory instance, so we ignore it.
            //Once the servers have been reconnected again, we will set the correct state of the
            //ManagedServer.
            return;
        }
