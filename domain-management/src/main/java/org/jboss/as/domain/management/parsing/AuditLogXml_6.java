 ...
 */
final class AuditLogXml_6 implements AuditLogXml {
    final boolean host;
    final Stability stability;

    AuditLogXml_6(boolean host, Stability stability) {
        this.host = host;
            ...
            switch (attribute) {
                ...
                case REDACTED: {
                    if (stability.enables(Stability.COMMUNITY)) {
                        AuditLogLoggerResourceDefinition.REDACTED.parseAndSetParameter(value, add, reader);
                    } else {
                        throw unexpectedAttribute(reader, i);
                    }
                    break;
    ...
    private void writeAuditLogger(XMLExtendedStreamWriter writer, ModelNode auditLog, String element) throws XMLStreamException {
        if (auditLog.hasDefined(element) && auditLog.get(element).hasDefined(ModelDescriptionConstants.AUDIT_LOG)){
            ...
            if (stability.enables(Stability.COMMUNITY)) {
                AuditLogLoggerResourceDefinition.REDACTED.marshallAsAttribute(config, writer);
            }
