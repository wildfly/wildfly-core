...
public class UpdateCommand extends AbstractInstMgrCommand {
    ...
    protected List<String> manifestVersions;
    @Option(name = "allow-manifest-downgrades", defaultValue = "false")
    protected boolean allowManifestDowngrades;
    ...
    protected boolean noResolveLocalCache;
    ...
    protected boolean useDefaultLocalCache;
    @Option(name = "offline", hasValue = false)
    protected boolean offline;
    @OptionList(name = "maven-repo-files")
    protected List<File> mavenRepoFiles;
     ...
     */
    private void assertOperationPropertySupported(CommandContext ctx, String operationName, String propertyName)
        ...
        try {
            ...
            if (!response.get(RESULT).get(REQUEST_PROPERTIES).hasDefined(propertyName)) {
                String message = String.format("The target host doesn't support parameter %s in operation %s:%s",
                        propertyName, address, operationName);
