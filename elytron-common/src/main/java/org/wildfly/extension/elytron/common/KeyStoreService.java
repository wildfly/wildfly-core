...
public class KeyStoreService implements ModifiableKeyStoreService {
    ...
    public static KeyStoreService createFileLessKeyStoreService(String provider, String type, String aliasFilter) {
        return new KeyStoreService(provider, type, null, null, false, aliasFilter);
    }

    public static KeyStoreService createFileBasedKeyStoreService(String provider, String type, String relativeTo, String path, boolean required, String aliasFilter) {
    ...
    public void start(StartContext startContext) throws StartException {
        try {
            ...
            if (resolvedPath != null && ! resolvedPath.exists()) {
                if (required) {
                    if (type == null) {
                        throw ElytronCommonMessages.ROOT_LOGGER.nonexistingKeyStoreMissingType();
                    } else {
                        throw ElytronCommonMessages.ROOT_LOGGER.keyStoreFileNotExists(resolvedPath.getAbsolutePath());
                    }
                } else {
                    ElytronCommonMessages.ROOT_LOGGER.keyStoreFileNotExistsButIgnored(resolvedPath.getAbsolutePath());
                }
            }
            ...
            try (FileInputStream is = (resolvedPath != null && resolvedPath.exists()) ? new FileInputStream(resolvedPath) : null) {
                ...
                if (is != null) {
                    if (type != null) {
                        ...
                        if (detected == null) {
                            throw ElytronCommonMessages.ROOT_LOGGER.unableToDetectKeyStore(resolvedPath.getPath());
                        }
                    ...
                    if (keyStore == null) {
                        ...
                        ElytronCommonMessages.ROOT_LOGGER.debugf(
            ...
            throw ElytronCommonMessages.ROOT_LOGGER.unableToStartService(e);
        }
    }
    ...
    private Provider resolveProvider() throws StartException {
        ...
        if (identified == null) {
            throw ElytronCommonMessages.ROOT_LOGGER.noSuitableProvider(type);
        }
        return identified;
    }
    ...
    private void checkCertificatesValidity(KeyStore keyStore) throws KeyStoreException {
        if (ElytronCommonMessages.ROOT_LOGGER.isEnabled(Logger.Level.WARN)) {
            ...
            while (aliases.hasMoreElements()) {
                ...
                if (certificate != null && certificate instanceof X509Certificate) {
                    try {
                        ...
                        ElytronCommonMessages.ROOT_LOGGER.certificateNotValid(alias, e);
                    }
                }
            }
        }
    ...
    public LoadKey load() throws OperationFailedException {
        try {
            ElytronCommonMessages.ROOT_LOGGER.tracef("reloading KeyStore from file [%s]", resolvedPath);
            ...
            throw ElytronCommonMessages.ROOT_LOGGER.unableToCompleteOperation(e, e.getLocalizedMessage());
        }
    }

    public void revertLoad(final LoadKey loadKey) {
        ElytronCommonMessages.ROOT_LOGGER.trace("reverting load of KeyStore");
    ...
    public void save() throws OperationFailedException {
        if (resolvedPath == null) {
            throw ElytronCommonMessages.ROOT_LOGGER.cantSaveWithoutFile(path);
        }
        ElytronCommonMessages.ROOT_LOGGER.tracef("saving KeyStore to the file [%s]", resolvedPath);
    ...
    public char[] resolveKeyPassword(final ExceptionSupplier<CredentialSource, Exception> keyPasswordCredentialSourceSupplier) throws Exception {
        ...
        if (cs == null) throw ElytronCommonMessages.ROOT_LOGGER.keyPasswordCannotBeResolved(path);
        PasswordCredential credential = cs.getCredential(PasswordCredential.class);
        if (credential == null) throw ElytronCommonMessages.ROOT_LOGGER.keyPasswordCannotBeResolved(path);
        ClearPassword password = credential.getPassword(ClearPassword.class);
        if (password == null) throw ElytronCommonMessages.ROOT_LOGGER.keyPasswordCannotBeResolved(path);
    ...
    private char[] resolvePassword() throws Exception {
        ...
        String path = resolvedPath != null ? resolvedPath.getPath() : "null";
        if (cs == null) throw ElytronCommonMessages.ROOT_LOGGER.keyStorePasswordCannotBeResolved(path);
        PasswordCredential credential = cs.getCredential(PasswordCredential.class);
        if (credential == null) throw ElytronCommonMessages.ROOT_LOGGER.keyStorePasswordCannotBeResolved(path);
        ClearPassword password = credential.getPassword(ClearPassword.class);
        if (password == null) throw ElytronCommonMessages.ROOT_LOGGER.keyStorePasswordCannotBeResolved(path);

        return password.getPassword();
    }

    public File getResolvedPath(PathResolver pathResolver, String path, String relativeTo) {
    ...
    public void generateAndSaveSelfSignedCertificate(String host, char[] password) {
        try {
            if (shouldAutoGenerateSelfSignedCertificate(host)) {
                ...
                ElytronCommonMessages.ROOT_LOGGER.selfSignedCertificateHasBeenCreated(resolvedPath.getAbsolutePath(), getShaFingerprint(selfSignedCertificate, "SHA-1"), getShaFingerprint(selfSignedCertificate, "SHA-256"));
                save();
            }
        } catch (Exception e) {
            throw ElytronCommonMessages.ROOT_LOGGER.failedToStoreGeneratedSelfSignedCertificate(e);
        }
    }

    public boolean shouldAutoGenerateSelfSignedCertificate(String host) {
    ...
    protected static class LoadKey {
        private final AtomicLoadKeyStore.LoadKey loadKey;
        private final long modifiedTime;
        private final boolean modified;

        protected LoadKey(AtomicLoadKeyStore.LoadKey loadKey, long modifiedTime, boolean modified) {
