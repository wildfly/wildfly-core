...
/**
 * Unit test for {@link EnumSetValidator}.
 * @author Paul Ferraro
 */
public class EnumSetValidatorTestCase {

    @Test
    public void allOf() {
        this.verify(new EnumSetValidator<>(ChronoUnit.class), EnumSet.allOf(ChronoUnit.class));
    }

    @Test
    public void of() {
        Set<ChronoUnit> allowed = EnumSet.of(ChronoUnit.SECONDS, ChronoUnit.MINUTES, ChronoUnit.HOURS);
        this.verify(new EnumSetValidator<>(allowed), allowed);
    }
    ...
    private void verify(ParameterValidator validator, Set<ChronoUnit> allowed) {
        for (ChronoUnit unit : EnumSet.allOf(ChronoUnit.class)) {
            ThrowingCallable validation = () -> validator.validateParameter("unit", new ModelNode(unit.name()));
            ...
            assertThatExceptionOfType(OperationFailedException.class).isThrownBy(() -> validator.validateParameter("unit", new ModelNode(unit.toString())));
        }
