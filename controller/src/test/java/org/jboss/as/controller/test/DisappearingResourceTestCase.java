...
public class DisappearingResourceTestCase extends AbstractControllerTestBase {
    ...
    public void testDisappearingTopLevelAttribute() throws Exception {
        ...
        assertTrue(rsp.toString(), rsp.get(FAILURE_DESCRIPTION).asString().contains(ControllerLogger.MGMT_OP_LOGGER.managementResourceNotFound(CHILD_B_ADDRESS)));
    }
    ...
    public void testResourceDisappearingInAssembly() throws Exception {
        ...
        ModelNode rsp = executeCheckForFailure(op);
        assertTrue(rsp.toString(), rsp.get(FAILURE_DESCRIPTION).asString().contains(ControllerLogger.MGMT_OP_LOGGER.managementResourceNotFound(PARENT_ADDRESS)));
    }
