                    ...
                    do {
                        if (group != null) {
                            if (!group.getNames().contains(reader.getName())) {
                                if (occurrences < group.getCardinality().getMinOccurs()) {
                                    throw ParseUtils.minOccursNotReached(reader, group.getAllNames(), group.getCardinality());
                                }
                        ...
                        if (group != null) {
                            ...
                            if (occurrences > group.getCardinality().getMaxOccurs().orElse(Integer.MAX_VALUE)) {
                                throw ParseUtils.maxOccursExceeded(reader, group.getAllNames(), group.getCardinality());
                            }
