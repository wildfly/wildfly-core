...
import org.jboss.as.controller.persistence.xml.ResourceXMLElementGroup;
...
public final class PersistentResourceXMLDescription implements ResourceParser, ResourceMarshaller {
    ...
    public static PersistentResourceXMLBuilder decorator(final String elementName) {
        ...
        return new PersistentResourceXMLBuilder(UNQUALIFIED_FACTORY, ResourceRegistration.of(PathElement.pathElement(elementName))) {
            ...
            public PersistentResourceXMLDescription build() {
                ...
                List<ResourceRegistrationXMLElement> children = childBuilders.stream().map(Supplier::get).map(PersistentResourceXMLDescription::getXMLElement).collect(Collectors.toList());
                ResourceXMLElement.Builder builder = UNQUALIFIED_FACTORY.element(UNQUALIFIED_FACTORY.resolve(elementName)).withCardinality(children.stream().anyMatch(element -> element.getCardinality().getMinOccurs() > 0) ? XMLCardinality.Single.REQUIRED : XMLCardinality.Single.OPTIONAL);
                boolean repeatableChildren = children.stream().map(ResourceRegistrationXMLElement::getCardinality).map(XMLCardinality::getMaxOccurs).allMatch(OptionalInt::isEmpty);
                ResourceXMLElementGroup group = repeatableChildren ? this.buildGroup(UNQUALIFIED_FACTORY.choice().withCardinality(XMLCardinality.Unbounded.OPTIONAL), children) : this.buildGroup(UNQUALIFIED_FACTORY.all(), children);
                ResourceXMLElement element = builder.withContent(XMLContent.of(group)).build();
                return new PersistentResourceXMLDescription(new ResourceRegistrationXMLElement() {
                    ...
                    @Override
                    public XMLElementReader<Entry<PathAddress, Map<PathAddress, ModelNode>>> getReader() {
                    ...
                    public Stability getStability() {
                        return element.getStability();
                    }
                });
            }

            private <G extends ResourceXMLElementGroup, B extends ResourceXMLElementGroup.Builder<G, B>> G buildGroup(B builder, List<ResourceRegistrationXMLElement> children) {
                for (ResourceRegistrationXMLElement child : children) {
    ...
    public static interface Builder {
        ...
        Builder setCardinality(XMLCardinality cardinality);
    ...
    public static class PersistentResourceXMLBuilder implements Builder {
        ...
        private PersistentResourceXMLBuilder(ResourceXMLParticleFactory factory, ResourceRegistration registration) {
            ...
            this.cardinality = registration.getPathElement().isWildcard() ? XMLCardinality.Unbounded.OPTIONAL : XMLCardinality.Single.OPTIONAL;
        }
        ...
        @Override
        public PersistentResourceXMLBuilder setCardinality(XMLCardinality cardinality) {
        ...
        public PersistentResourceXMLDescription build() {
            ...
            XMLElement<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> wrapperElement = this.factory.element(this.factory.resolve(wrapperLocalName))
                    .withCardinality(element.getCardinality().getMinOccurs() > 0 ? XMLCardinality.Single.REQUIRED : XMLCardinality.Single.OPTIONAL)
                    .withContent(XMLContent.of(XMLParticleGroup.singleton(element)))
                    .build();
            return new PersistentResourceXMLDescription(new ResourceRegistrationXMLElement() {
                @Override
                public PathElement getPathElement() {
                ...
                @Override
                public Stability getStability() {
                    return wrapperElement.getStability();
                }
            });
        }
        ...
        private <E extends ResourceRegistrationXMLElement, B extends ResourceRegistrationXMLElement.Builder<E, B>> E build(B builder) {
            ...
            builder.withCardinality(this.cardinality).withOperationTransformation(this.operationTransformation);
            List<AttributeDefinition> elements = new LinkedList<>();
            Map<String, List<AttributeDefinition>> groupElements = this.useElementsForGroups ? new LinkedHashMap<>() : Map.of();
            for (AttributeDefinition attribute : this.attributes) {
                ...
                if (groupName != null) {
                    List<AttributeDefinition> groupAttributes = groupElements.get(groupName);
                    if (groupAttributes == null) {
                        groupAttributes = new LinkedList<>();
                        groupElements.put(groupName, groupAttributes);
                    }
            ...
            // Otherwise, PersistentResourceXMLDescription expects child element types in following order:
            // 1. attribute elements
            // 2. attribute group elements
            // 3. child resources
            // However, most subsystems rely on unordered semantics, where xs:sequence enforcement would break
            List<ResourceRegistrationXMLElement> children = this.children.stream().map(Supplier::get).map(PersistentResourceXMLDescription::getXMLElement).collect(Collectors.toList());
            boolean repeatableElements = elements.stream().map(attribute -> this.parsers.getOrDefault(attribute, attribute.getParser()).getCardinality(attribute)).map(XMLCardinality::getMaxOccurs).allMatch(OptionalInt::isEmpty);
            boolean repeatableChildren = children.stream().map(ResourceRegistrationXMLElement::getCardinality).map(XMLCardinality::getMaxOccurs).allMatch(OptionalInt::isEmpty);
            ResourceXMLElementGroup group = repeatableChildren || repeatableElements ? (!groupElements.isEmpty() ? this.buildGroup(this.factory.sequence().withCardinality(XMLCardinality.Single.OPTIONAL), elements, groupElements, children) : this.buildGroup(this.factory.choice().withCardinality(XMLCardinality.Unbounded.OPTIONAL), elements, groupElements, children)) : this.buildGroup(this.factory.all().withCardinality(XMLCardinality.Single.OPTIONAL), elements, groupElements, children);
            return builder.withContent(XMLContent.of(group)).build();
        }

        private <G extends ResourceXMLElementGroup, B extends ResourceXMLElementGroup.Builder<G, B>> G buildGroup(B builder, List<AttributeDefinition> elements, Map<String, List<AttributeDefinition>> groupElements, List<ResourceRegistrationXMLElement> children) {
            builder.withParsers(this.parsers).withMarshallers(this.marshallers);
            builder.addElements(elements);
            for (Map.Entry<String, List<AttributeDefinition>> groupEntry : groupElements.entrySet()) {
                builder.addElement(groupEntry.getKey(), groupEntry.getValue());
            }
            for (ResourceRegistrationXMLElement child : children) {
                builder.addElement(child);
            }
