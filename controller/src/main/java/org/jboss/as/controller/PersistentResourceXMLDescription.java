...
public class PersistentResourceXMLDescription {
    ...
    private String parseAttributes(final XMLExtendedStreamReader reader, ModelNode op, Map<String, AttributeDefinition> attributes, boolean wildcard) throws XMLStreamException {
        ...
        /*Map<String,AttributeDefinition> elements =  attributes.values().stream()
                        .filter(a -> a.getParser().isParseAsElement())
                .collect(Collectors.toMap(AttributeDefinition::getXmlName,
                                        Function.identity()));*/

        /*for (AttributeDefinition ad : attributes.values().stream()
                .filter(a -> a.getParser().isParseAsElement() )
