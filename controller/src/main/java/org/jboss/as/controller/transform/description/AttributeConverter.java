    ...
    AttributeConverter DEFAULT_VALUE = new AttributeConverter() {
        ...
        public void convertOperationParameter(PathAddress address, String attributeName, ModelNode attributeValue, ModelNode operation, TransformationContext context) {
            if (!attributeValue.isDefined()) {
                ...
                AttributeDefinition definition = this.findOperationParameter(registration, operation, attributeName);
                attributeValue.set(definition.getDefaultValue());
            }
        }

        private AttributeDefinition findOperationParameter(ImmutableManagementResourceRegistration registration, ModelNode operation, String attributeName) {
            ...
            // Operation definitions created by SimpleResourceDefinition.registerAddOperation(...) have no parameters, so fetch attribute definition from resource definition instead
            return registration.getAttributeAccess(PathAddress.EMPTY_ADDRESS, attributeName).getAttributeDefinition();
        }
