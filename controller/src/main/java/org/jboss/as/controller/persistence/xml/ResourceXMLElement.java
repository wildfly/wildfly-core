...
public interface ResourceXMLElement extends ResourceRegistration, ResourceXMLChoice, XMLElement<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> {
    ...
    interface Builder extends QNameResolver {
        ...
        /**
         * Overrides the cardinality of this resource.
         * @return a reference to this builder.
         */
        ...
        /**
         * Adds a child resource to this description to be written <em>before</em> any element-based attributes of this resource.
        ...
        /**
         * Adds a child resource to this description to be written <em>after</em> any element-based attributes of this resource.
         * @param description an XML description of a child resource.
         * @return a reference to this builder.
         */
        Builder addChild(XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> choice);

        /**
         * Adds a child resource to this description to be written <em>before</em> any element-based attributes of the specified attribute group.
        ...
        /**
         * Adds a child resource to this description to be written <em>after</em> any element-based attributes of the specified attribute group.
         * @param attribute a grouped attribute.
         * @param choice a choice.
         * @return a reference to this builder.
         */
        Builder addChild(AttributeDefinition attribute, XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> choice);
        ...
        /**
         * Indicates that the child elements of this resource should be parsed using xs:sequence semantics.
         * @return a reference to this builder.
         */
        Builder withSequentialChildren();

        /**
         * Indicates that the child elements of the element for the specified attribute group should be parsed using xs:sequence semantics.
         * @return a reference to this builder.
         */
        Builder withSequentialChildren(AttributeDefinition attribute);
    ...
    static class DefaultBuilder implements Builder {
        ...
        static AttributeDefinition createIgnoredAttributeDefinition(String localName) {
            return createIgnoredAttributeDefinition(localName, IGNORED_PARSER);
        }
        private static AttributeDefinition createIgnoredAttributeDefinition(String localName, AttributeParser parser) {
        ...
        volatile XMLCardinality cardinality;
        volatile Set<String> sequentialChildGroups = Set.of(); // The set of attribute groups containing sequential children
        volatile boolean sequentialChildren = false;
        ...
        protected DefaultBuilder(FeatureFilter filter, ResourceDescription description, QNameResolver qualifiedNameFactory) {
            ...
            this.cardinality = wildcard ? XMLCardinality.Unbounded.OPTIONAL : XMLCardinality.Single.OPTIONAL;
        ...
        public Builder require() {
            return this.withCardinality(this.description.getPathElement().isWildcard() ? XMLCardinality.Unbounded.REQUIRED : XMLCardinality.Single.REQUIRED);
        }
        ...
        @Override
        public Builder addChild(AttributeDefinition attribute, XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> choice) {
            String group = attribute.getAttributeGroup();
            Assert.assertNotNull(group);
        ...
        @Override
        public Builder addChild(XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> choice) {
        ...
        @Override
        public Builder withSequentialChildren() {
            this.sequentialChildren = true;
            return this;
        }

        @Override
        public Builder withSequentialChildren(AttributeDefinition attribute) {
            String group = attribute.getAttributeGroup();
            if (group != null) {
                if (this.sequentialChildGroups.isEmpty()) {
                    this.sequentialChildGroups = new TreeSet<>();
                }
                this.sequentialChildGroups.add(group);
            }
            return this;
        }
        ...
        public ResourceXMLElement build() {
            ...
            AttributeDefinition pathValueAttribute = (pathValueAttributeName != null) ? createIgnoredAttributeDefinition(UUID.randomUUID().toString(), NO_OP_PARSER) : null;
            ...
            Set<QName> sequentialGroups = !this.sequentialChildGroups.isEmpty() ? this.sequentialChildGroups.stream().map(groupElementNames).collect(Collectors.toSet()) : Set.of();
            ...
            while (resourceAttributes.hasNext()) {
                ...
                if (parser.isParseAsElement() || marshaller.isMarshallableAsElement()) {
                    ...
                    if (existing != null) {
                        throw ControllerLogger.ROOT_LOGGER.duplicateAttributes(Optional.ofNullable(attributeGroupName).orElse(name), attributeName);
                    }
                }
            }
            ...
            if (!groups.isEmpty()) {
                ...
                for (Map.Entry<QName, Map.Entry<Map<QName, AttributeDefinition>, List<XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode>>>> groupEntry : groups.entrySet()) {
                    QName groupName = groupEntry.getKey();
                    Map<QName, AttributeDefinition> groupAttributes = groupEntry.getValue().getKey();
                    List<XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode>> groupChildren = groupEntry.getValue().getValue();
                    ...
                    XMLContent<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> childContent = sequentialGroups.contains(groupName) ? XMLContent.sequence(groupChildren) : XMLContent.all(groupChildren);
                    defaultGroup.getValue().add(new ResourceOperationXMLElement<>(groupName, attributesReader, attributesWriter, Function.identity(), childContent));
                }
            }
            ...
            XMLCardinality cardinality = this.cardinality;

            Map<QName, AttributeDefinition> defaultGroupAttributes = defaultGroup.getKey();
            List<XMLChoice<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode>> defaultGroupChildren = defaultGroup.getValue();
            ...
            XMLContent<Map.Entry<PathAddress, Map<PathAddress, ModelNode>>, ModelNode> childContent = this.sequentialChildren ? XMLContent.sequence(defaultGroupChildren) : XMLContent.all(defaultGroupChildren);
