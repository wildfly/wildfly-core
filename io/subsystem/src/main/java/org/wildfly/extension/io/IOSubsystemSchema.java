...
public enum IOSubsystemSchema implements SubsystemResourceXMLSchema<IOSubsystemSchema> {
    ...
    public SubsystemResourceRegistrationXMLElement getSubsystemXMLElement() {
        ...
        if (this.since(VERSION_4_0)) {
            ...
            builder.withOperationTransformation(new UnaryOperator<>() {
                ...
                public ModelNode apply(ModelNode operation) {
                    ...
                    operation.get(IOSubsystemResourceDefinitionRegistrar.DEFAULT_WORKER.getName()).set(IOSubsystemResourceDefinitionRegistrar.LEGACY_DEFAULT_WORKER);
                    return operation;
                }
            });
        }
        NamedResourceRegistrationXMLElement.Builder workerBuilder = this.factory.namedElement(ResourceRegistration.of(WorkerResourceDefinition.PATH))
        ...
        if (this.since(VERSION_2_0)) {
            ResourceRegistrationXMLElement outboundBindAddressElement = this.factory.namedElement(ResourceRegistration.of(OutboundBindAddressResourceDefinition.PATH))
                    .addAttributes(OutboundBindAddressResourceDefinition.ATTRIBUTES)
                    .build();
            workerBuilder.withContent(XMLContent.of(this.factory.choice().withCardinality(XMLCardinality.Unbounded.OPTIONAL).addElement(outboundBindAddressElement).build()));
        }
        ResourceXMLChoice content = this.factory.choice().withCardinality(XMLCardinality.Unbounded.REQUIRED)
                ...
                .addElement(this.factory.namedElement(ResourceRegistration.of(BufferPoolResourceDefinition.PATH)).addAttributes(BufferPoolResourceDefinition.ATTRIBUTES).build())
                .build();
