...
import org.jboss.as.controller.persistence.xml.WildcardResourceRegistrationXMLElement;
...
public enum IOSubsystemSchema implements SubsystemResourceXMLSchema<IOSubsystemSchema> {
    ...
    public SubsystemResourceRegistrationXMLElement getSubsystemXMLElement() {
        SubsystemResourceRegistrationXMLElement.Builder builder = this.factory.element(IOSubsystemResourceRegistrar.INSTANCE);
        ...
        WildcardResourceRegistrationXMLElement.Builder workerBuilder = this.factory.element(WildcardResourceRegistration.of(WorkerResourceDefinition.PATH)).addAttributes(List.of(
        ...
        if (this.since(VERSION_2_0)) {
            workerBuilder.withContent(XMLContent.of(XMLParticleGroup.singleton(this.factory.element(WildcardResourceRegistration.of(OutboundBindAddressResourceDefinition.PATH)).addAttributes(OutboundBindAddressResourceDefinition.ATTRIBUTES).build())));
        }
        ResourceXMLChoice content = this.factory.choice().withCardinality(XMLCardinality.Unbounded.REQUIRED)
                ...
                .addElement(this.factory.element(WildcardResourceRegistration.of(BufferPoolResourceDefinition.PATH)).addAttributes(BufferPoolResourceDefinition.ATTRIBUTES).build())
                .build();
