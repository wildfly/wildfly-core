...
public enum ElytronSubsystemSchema implements PersistentSubsystemSchema<ElytronSubsystemSchema> {
    ...
    VERSION_18_1_COMMUNITY(18, 1, Stability.COMMUNITY),
    ;
    static final Map<Stability, ElytronSubsystemSchema> CURRENT = Feature.map(EnumSet.of(VERSION_18_0, VERSION_18_1_COMMUNITY));
    ...
    ElytronSubsystemSchema(final int major, final int minor, final Stability stability) {
        this.namespace = SubsystemSchema.createSubsystemURN(ElytronExtension.SUBSYSTEM_NAME, stability, new IntVersion(major, minor));
    }
    ...
    private void addTlsParser(PersistentResourceXMLDescription.PersistentResourceXMLBuilder builder) {
        ...
        if (this.since(ElytronSubsystemSchema.VERSION_18_1_COMMUNITY)) {
            if(this.enables(getDynamicClientSSLContextDefinition())) {
                builder.addChild(tlsParser.tlsParserCommunity_18_1_DynamicClientSSL);
            } else {
                builder.addChild(tlsParser.tlsParserCommunity_18_1);
            }
