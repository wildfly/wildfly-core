    ...
    static CertificateValidity getValidity(final Date notBefore, final Date notAfter, final long expirationWaterMark) {
        ...
        if (currentDate.before(notBefore)) {
            ...
            if (expirationWaterMark > 0) {
                ...
                final Date interimBoundryDate = new Date(currentDate.getTime() + Duration.of(expirationWaterMark, ChronoUnit.MINUTES).toMillis());
                ...
                final long WEEK_IN_MS = Duration.of(1, ChronoUnit.WEEKS).toMillis();
                final long DAY_IN_MS = Duration.of(1, ChronoUnit.DAYS).toMillis();
