...
public class DomainSerialFilterScriptTestCase extends ScriptTestCase {
    ...
    private static final PathAddress HOST_PRIMARY = PathAddress.pathAddress("host", "primary");
    private static final PathAddress SERVER_SERVER_ONE = PathAddress.pathAddress("server", "server-one");
    ...
    public DomainSerialFilterScriptTestCase() {
        super(SCRIPT_NAME);
    }
    ...
    void testScript(ScriptProcess script) throws InterruptedException, TimeoutException, IOException {
        ...
        if (env.containsKey("JAVA_OPTS")) {
            ...
            Assert.assertTrue("Expected to find the @jdk.serialFilter path in the JVM parameters for a server started with " + script.getLastExecutedCmd() + "\nThe server output was: \n" + stdout, ok);
        }
    }

    boolean propertiesCheck(final ModelControllerClient client) {
        boolean running = HOST_CONTROLLER_CHECK.apply(client);
        if (!running) {
            return false;
        }
        if (env.isEmpty()) {
            ...
            try {
                ModelNode op = Util.getReadAttributeOperation(HOST_PRIMARY.append(CORE_SERVICE_PLATFORM_MBEAN).append(TYPE_RUNTIME), "input-arguments");
                ModelNode result = client.execute(op);
                checkSuccessful(result);
                result = result.get(ClientConstants.RESULT);
                if (result.asString().contains("-Djdk.serialFilter=")) {
                    op = Util.getReadAttributeOperation(HOST_PRIMARY.append(SERVER_SERVER_ONE).append(CORE_SERVICE_PLATFORM_MBEAN)
                            .append(TYPE_RUNTIME), "input-arguments");
