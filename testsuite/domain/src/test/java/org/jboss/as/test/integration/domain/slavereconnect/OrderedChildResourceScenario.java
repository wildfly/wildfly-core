...
public class OrderedChildResourceScenario extends ReconnectTestScenario {
    ...
    private ModelNode compareSubsystemModels(DomainClient masterClient, DomainClient slaveClient, boolean serverReload) throws Exception {
        ...
        String status = DomainTestUtils.executeForResult(Util.getReadAttributeOperation(SLAVE_SERVER_ADDRESS, "runtime-configuration-state"), masterClient).asString();
        ...
        Assert.assertEquals(serverReload ? RELOAD_REQUIRED : "ok", status);
        return domain;
    }
