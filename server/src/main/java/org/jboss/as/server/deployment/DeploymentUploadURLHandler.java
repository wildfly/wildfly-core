...
public class DeploymentUploadURLHandler
    ...
    private static InputStream openAuthenticatedStream(URL url, AuthenticationContext authCtx) throws IOException {
        try {
            URI uri = url.toURI();
        ...
        try {
            ...
            if (e.getCallback() == credentialCallback) {
                ...
                try {
                    ...
                    ServerLogger.ROOT_LOGGER.tracef(ex, "Unable to obtain password for deployment URL authentication");
                }
            }
        } catch (Exception e) {
            ServerLogger.ROOT_LOGGER.tracef(e, "Unable to obtain credentials for deployment URL authentication");
        }
