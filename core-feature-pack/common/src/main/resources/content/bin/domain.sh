...
while true; do
   ...
   if [ "$JBOSS_STATUS" -eq 10 ]; then
      ...
      "${JBOSS_HOME}/bin/_installation-manager_helper.sh" "${JBOSS_HOME}" "${JBOSS_CONFIG_DIR}/logging.properties" "${JBOSS_LOG_DIR}/host-controller.log" >> "${INST_MGR_CONSOLE_FILE}" 2>&1
      if [ $? -eq 0 ]; then
