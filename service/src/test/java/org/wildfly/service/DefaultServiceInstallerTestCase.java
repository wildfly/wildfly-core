...
public class DefaultServiceInstallerTestCase {
    ...
    public void test() throws StartException {
        ...
        ServiceInstaller installer = ServiceInstaller.builder(mapper, dependency).provides(name).withDependency(dependency).onStart(startTask).onStop(stopTask).build();
        ...
        DefaultServiceTestCase.test(service.getValue(), "value", "mappedValue", List.of(injector, startTask), mapper, dependency, stopTask);
    }
}
